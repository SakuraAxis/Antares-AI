...
struct HighlightsShadowsParams {
    width: u32,
    height: u32,
    strength: f32,   // abs(amount) / 100.0
    amount_sign: f32, // amount / 100.0 ( sign determines shadow lift vs highlight recovery )
}

#[derive(Clone)]
pub struct HighlightsShadowsPipeline {
    pipeline: wgpu::ComputePipeline,
    bind_group_layout: wgpu::BindGroupLayout,
}

impl HighlightsShadowsPipeline {
    pub fn new(device: &wgpu::Device) -> Self {
        // Load common color functions and highlights/shadows-specific shader
        let color_common = include_str!("../shaders/common/color.wgsl");
        let hs_shader = include_str!("../shaders/highlights_shadows.wgsl");
        let shader_source = format!("{}\n\n{}", color_common, hs_shader);

        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("highlights_shadows_shader"),
            source: wgpu::ShaderSource::Wgsl(shader_source.into()),
        });

        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("highlights_shadows_bind_group_layout"),
            entries: &[
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("highlights_shadows_pipeline_layout"),
            bind_group_layouts: &[Some(&bind_group_layout)],
            immediate_size: 0,
        });

        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("highlights_shadows_pipeline"),
    ...
    pub async fn apply(
        ...
        let uniform_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
            label: Some("hs_params"),
            contents: bytemuck::bytes_of(&params),
            usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
        });

        let staging_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("hs_staging"),
