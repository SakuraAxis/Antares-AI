...
impl BrightnessPipeline {
    pub fn new(device: &wgpu::Device) -> Self {
        ...
        let brightness_shader = include_str!("../shaders/brightness.wgsl");
        let shader_source = format!("{}\n\n{}", color_common, brightness_shader);

        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("brightness_shader"),
            source: wgpu::ShaderSource::Wgsl(shader_source.into()),
        });

        let bind_group_layout = device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
            label: Some("brightness_bind_group_layout"),
            entries: &[
        ...
        let pipeline_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("brightness_pipeline_layout"),
            bind_group_layouts: &[Some(&bind_group_layout)],
            immediate_size: 0,
        });

        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            label: Some("brightness_pipeline"),
    ...
    pub async fn apply(
        ...
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("brightness_encoder"),
        });

        {
            let mut pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("brightness_pass"),
